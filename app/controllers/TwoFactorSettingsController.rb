class TwoFactorSettingsController < ApplicationController
    ...
    def new
      if current_user.otp_required_for_login
        flash[:alert] = 'Two factor auth is already enabled.'
    ...
    def create
      ...
      if current_user.validate_and_consume_otp!(enable_2fa_params[:code])
        ...
        flash[:notice] = 'Enabled two factor auth, keep your backup codes in a safe place.'
    ...
    def edit
      unless current_user.otp_required_for_login
        flash[:alert] = 'Please enable two factor auth first.'
    ...
    def destroy
      if current_user.disable_two_factor!
        flash[:notice] = 'Successfully disabled two factor auth.'
        redirect_to edit_user_registration_path
      else
        flash[:alert] = 'Could not disable two factor auth.'
